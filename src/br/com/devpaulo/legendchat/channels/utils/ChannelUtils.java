...
public class ChannelUtils {
    ...
    public static void realMessage(Channel c, Player sender, String message, String bukkit_format, boolean cancelled) {
        ...
        if (Legendchat.isCensorActive()) {
            ...
            if (message.equalsIgnoreCase("Ad!")) {
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cSua mensagem parece conter anúncios.. tente novamente!"));
                return;
            }
        }
        ...
        if (!Main.block_econ && c.getMessageCost() > 0) {
            ...
            if (!sender.hasPermission("legendchat.channel." + c.getName().toLowerCase() + ".free") && !sender.hasPermission("legendchat.admin")) {
                if (custo > 0) {
                    if (Main.econ.getBalance(sender.getName()) < custo) {
                        sender.sendMessage(Legendchat.getMessageManager().getMessage("error3").replace("@price", Double.toString(custo)));
                        return;
                    }
                    Main.econ.withdrawPlayer(sender.getName(), custo);
                    gastou = true;
                }
            }
        }
